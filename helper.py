...
def compare_models_with_numerical_features(df, target_col, feature_cols, test_size=0.2, random_state=42):
  """
  Function to compare the fit of different numerical features to explain the target variable. The function was built to take only numerical features.
  
  Args:
    df: Dataframe containing the data
    target_col: Name of the target column.
    feature_cols: List of numerical feature column names to compare.
    test_size: Proportion of data to use for testing, in this case I am using 20%.
    random_state: Seed for random number generator for reproducibility.

  Returns:
    A dictionary where keys are feature names and values are dictionaries containing 
    model summary statistics (e.g., R-squared, adjusted R-squared, AIC, BIC).
  """
